...
function MatchDashboard({ match: m, onBack }) {
  const [tab, setTab]         = useState("slots"); // slots | info | substitute
  ...
  const addBot = () => {
    ...
    setPlayers(prev => [...prev, { id:"b"+Date.now(), name:botForm.name, pos:botForm.pos, avatar:botForm.name[0].toUpperCase(), type:"bot" }]);
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.dashHeader}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <span style={{ ...S.statusPill, ...STATUS_CFG[m.status] }}>
              ...
              <div key={ti} style={S.teamCol}>
                <div style={S.teamHeader}>
                  ...
                  <span style={S.teamScore}>
                    Nível {team.length ? (team.length * 2.8 / team.length).toFixed(1) : "–"}
                  </span>
                </div>
                {team.map(p => (
                  <PlayerRow key={p.id} player={p} isOrg={isOrg} onRemove={() => removePlayer(p.id)}/>
                ))}
