...
export const radarService = {
  ...
  /**
   * Busca jogadores que estão ativamente procurando partida (seeking_profiles).
   * Junta com player_profiles para obter nome, posição e skill.
   *
   * @param {{ position?: string, search?: string }} opts
   * @returns {Promise<Array>}
   */
  async getSeekingPlayers({ position, search } = {}) {
    let query = supabase
      .from('seeking_profiles')
      .select(`
        *,
        player_profiles!seeking_profiles_user_id_fkey(
          display_name, primary_position, skill_score, total_matches, bio, city
        )
      `)
      .eq('is_active', true)
    ...
    // Filtro de busca por nome ou bio (client-side — seeking_profiles não tem FTS)
    if (search && search.trim()) {
      const q = search.trim().toLowerCase()
      results = results.filter(sp => {
        const name = sp.player_profiles?.display_name?.toLowerCase() || ''
        const bio  = (sp.bio || sp.player_profiles?.bio || '').toLowerCase()
