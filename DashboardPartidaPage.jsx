...
const MOCK_MATCH = {
  ...
  organizer_id: 'dev-user', // ← usado para verificar se é admin
  ...
  rsvps: [
    ...Array.from({ length: 7 }, (_, i) => ({
      ...
      user: { player_profiles: [{ display_name: `Jogador ${i + 1}`, primary_position: 'MIDFIELDER' }] },
    })),
    // Dois suplentes convidados (INVITED) — aguardando confirmação de presença
    { id: 'r-s1', status: 'INVITED', user_id: 'sub-1', is_substitute: true,
      user: { player_profiles: [{ display_name: 'Carlos Suplente', primary_position: 'FORWARD' }] } },
    { id: 'r-s2', status: 'INVITED', user_id: 'sub-2', is_substitute: true,
      user: { player_profiles: [{ display_name: 'Diego Reserva', primary_position: 'DEFENDER' }] } },
  ],
}
...
function PlayerPresenceRow({ rsvp, isAdmin, onMarkNoShow, onConfirmGuest, loadingId }) {
  ...
  return (
    <div style={{
        ...
        <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
          {isInvited && (
            <button
              ...
              disabled={isLoading}
              title="Confirmar presença do suplente"
              style={{
                background: 'rgba(28,184,91,0.12)', color: '#1cb85b',
                border: '1px solid rgba(28,184,91,0.25)',
          ...
          <button
            ...
            disabled={isLoading}
            title="Registrar falta deste jogador"
            style={{
      ...
      {/* Badge status (não-admin ou já confirmado sem ações) */}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ background: 'var(--surface-container)', borderRadius: 20, padding: 20 }}>
        ...
        {embedUrl ? (
          ...
          <div
            style={{ width: '100%', height: 180, background: 'var(--surface-container-highest)', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 8, color: 'var(--on-surface-variant)', cursor: 'pointer' }}
          ...
          >
            ...
            <p style={{ fontSize: 12, fontWeight: 600 }}>
              {address ? 'Toque para abrir no Maps' : 'Endereço não informado'}
            </p>
          </div>
        )}
      </div>
...
export default function DashboardPartidaPage() {
  ...
  // Controle de presença (admin): { [rsvpId]: true } quando operação em andamento
  const [presenceLoading, setPresenceLoading] = useState(null)
  // Feedback de ação admin
  const [presenceFeedback, setPresenceFeedback] = useState(null) // { message, type }
  ...
  async function handleMarkNoShow(rsvp) {
    ...
    try {
      ...
      setMatch(prev => ({
        ...prev,
        rsvps: prev.rsvps.map(r => r.id === rsvp.id ? { ...r, status: 'NO_SHOW' } : r),
      }))
      showFeedback(`Falta registrada para ${rsvp.user?.player_profiles?.[0]?.display_name || 'jogador'}.`, 'error')
    } catch {
  ...
  // Admin: confirma chegada de suplente convidado
  async function handleConfirmGuest(rsvp) {
    ...
    try {
      ...
      setMatch(prev => ({
        ...prev,
        rsvps: prev.rsvps.map(r => r.id === rsvp.id ? { ...r, status: 'CONFIRMED', is_substitute: true } : r),
      }))
      showFeedback(`${rsvp.user?.player_profiles?.[0]?.display_name || 'Suplente'} confirmado! ✅`, 'success')
    } catch {
        ...
        <div style={{ background: 'var(--surface-container-low)', padding: '24px 20px 0' }}>
          ...
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 20 }}>
            ...
            <span style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#1cb85b', fontSize: 13, fontWeight: 700 }}>
              ...
              {match.rsvps?.filter(r => r.status === 'CONFIRMED').length ?? 0}/{match.total_slots} confirmados
            </span>
          </div>
        ...
        <div style={{ padding: '20px' }}>
          ...
          {tab === 'escala' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {presenceFeedback && (
                <div style={{
                  background: presenceFeedback.type === 'success' ? 'rgba(28,184,91,0.1)' : 'rgba(255,115,81,0.1)',
                  border: `1px solid ${presenceFeedback.type === 'success' ? 'rgba(28,184,91,0.25)' : 'rgba(255,115,81,0.25)'}`,
                  color: presenceFeedback.type === 'success' ? '#1cb85b' : '#ff7351',
                  borderRadius: 12, padding: '10px 14px', fontSize: 13, fontWeight: 700,
                  display: 'flex', alignItems: 'center', gap: 8,
                  animation: 'fadeIn 0.2s ease',
                }}>
                  <span className="material-symbols-outlined" style={{ fontSize: 18, fontVariationSettings: "'FILL' 1" }}>
                    {presenceFeedback.type === 'success' ? 'check_circle' : 'info'}
                  </span>
              ...
              {/* Banner admin */}
              {isAdmin && (
                <div style={{ background: 'rgba(178,153,255,0.08)', border: '1px solid rgba(178,153,255,0.2)', borderRadius: 12, padding: '10px 14px', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 8, color: '#b299ff', fontSize: 12, fontWeight: 700 }}>
              ...
              {/* Confirmados */}
              <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--on-surface-variant)', marginBottom: 6, marginTop: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {confirmedRsvps
                .filter(r => r.status === 'CONFIRMED' || r.status === 'NO_SHOW')
                .map((r, i) => (
                  <PlayerPresenceRow
                    key={r.id || i}
                    rsvp={r}
